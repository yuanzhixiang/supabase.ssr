...
import { CookieOptionsWithName } from "./types";
import { isBrowser } from "./utils/helpers";
import { combineChunks, createChunks, isChunkLike } from "./utils/chunker";
import { stringFromBase64URL, stringToBase64URL } from "./utils/base64url";
...
export function createStorageFromOptions(
  ...
  if (cookies) {
    if ("get" in cookies) {
      ...
      // 设置 getAll 方法，使用 get 方法构建 getAll 方法，getWithHints 是实现
      getAll = async (keyHints: string[]) => await getWithHints(keyHints);

      if ("set" in cookies && "remove" in cookies) {
        // 设置 setAll 方法，使用 set 和 remove 方法构建 setAll 方法
        ...
        // 如果 isServerClient 为 true，则设置 setAll 方法为空函数
        setAll = async () => {
          console.warn(
    ...
    // 如果有 getAll 方法，那么必须有 setAll 方法
      ...
      throw new Error(
        `@supabase/ssr: ${
          isServerClient ? "createServerClient" : "createBrowserClient"
    ...
    // 在浏览器里面，则使用 document.cookie API 实现 getAll 和 setAll 方法
    const noHintGetAll = () => {
    ...
    return {
      ...
      storage: {
        ...
        getItem: async (key: string) => {
          ...
          // 这个里面就是依赖下面的 async (chunkName: string) => { 方法去找对应的 cookie 值
